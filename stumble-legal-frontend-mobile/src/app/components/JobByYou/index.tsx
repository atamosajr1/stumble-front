...
export const JobByYou: FC<RequestByYouProps> = ({
  type,
  isMine,
  status,
  dttm,
  ...
  const buttonByStatus: ButtonProps[] = useMemo(() => {
    return [{ ...JOB_BUTTONS.CREATED_BY_YOU.PENDING[0], onPress: onOpenModal }];
  }, []);

  const popupBystatus = useMemo(() => {
    return {
      ...JOB_POPUP.CREATED_BY_YOU.PENDING,
      style: [
        {
          ...JOB_POPUP.CREATED_BY_YOU.PENDING.style[0],
          onPress: () => {
            onDeletePress();
            onCloseModal();
          },
        ...
        {
          ...JOB_POPUP.CREATED_BY_YOU.PENDING.style[1],
